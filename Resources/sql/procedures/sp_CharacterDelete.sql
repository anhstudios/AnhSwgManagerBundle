...
BEGIN
    ...
    WHERE A.object_id = character_id INTO acc_id;
    
    IF account_id = acc_id THEN

        UPDATE characters SET archived = 1 WHERE object_id = character_id;

        UPDATE characters SET deletedAt = (NOW()) WHERE object_id = character_id;
	...
	SELECT COUNT(*) from characters WHERE object_id = character_id AND archived = 1 INTO check_value;
